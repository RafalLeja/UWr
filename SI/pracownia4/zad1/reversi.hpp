...
Board makeMove(Board &B, pair<int, int> move, bool player){
    int x = move.first;
    int y = move.second;
    B.board[x][y] = player ? '#' : 'O';
    B.free.erase({x, y});
    for(int dx = -1; dx <= 1; dx++){
        for(int dy = -1; dy <= 1; dy++){
            ...
            if(canBeat(B, x, y, dx, dy, player)){
                int i = x + dx;
                int j = y + dy;
                while(B.board[i][j] == (!player ? '#' : 'O')){
                    B.board[i][j] = player ? '#' : 'O';
                    i += dx;
                    j += dy;
                }
            }
...
int heuristic(const Board &B){
    ...
    // score += corners(B, true) * 300;
    // score += mobility(board, true) * 5;

    return score;
}

bool terminal(const Board &B){
    return getMoves(B, true).empty() && getMoves(B, false).empty();
}
