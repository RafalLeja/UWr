...
__global__ void GPU_laplace3d(int NX, int NY, int NZ,
                              const T *__restrict__ d_u1,
                              T *__restrict__ d_u2) {
  int i, j, k, IOFF, JOFF, KOFF;
  long long indg;
  T u2, sixth = 1.0f / 6.0f;
...
int main(int argc, const char **argv) {
  ...
  int NX = 512, NY = 512, NZ = 512, REPEAT = 20, BLOCK_X, BLOCK_Y, bx, by, i, j,
      k;
  float *h_u1, *h_u2, *h_foo, *d_u1, *d_u2, *d_foo;
  double *h_du1, *h_du2, *h_dfoo, *d_du1, *d_du2, *d_dfoo;
  ...
  h_du1 = (double *)malloc(sizeof(double) * NX * NY * NZ);
  h_du2 = (double *)malloc(sizeof(double) * NX * NY * NZ);
  checkCudaErrors(cudaMalloc((void **)&d_du1, 2 * bytes));
  checkCudaErrors(cudaMalloc((void **)&d_du2, 2 * bytes));
  // initialise u1
  ...
  checkCudaErrors(cudaMemcpy(d_du1, h_du1, sizeof(double) * NX * NY * NZ,
  ...
  printf("%dx GPU_laplace3d (float): %.1f (ms) \n\n", REPEAT, milli);

  cudaEventRecord(start);

  for (i = 0; i < REPEAT; i++) {
    GPU_laplace3d<<<dimGrid, dimBlock>>>(NX, NY, NZ, d_du1, d_du2);
    getLastCudaError("GPU_laplace3d execution failed\n");
  ...
  cudaEventElapsedTime(&milli, start, stop);
  printf("Block dimensions: %d x %d\n", BLOCK_X, BLOCK_Y);
  printf("%dx GPU_laplace3d (double): %.1f (ms) \n\n", REPEAT, milli);

  // Read back GPU results

  cudaEventRecord(start);
  checkCudaErrors(cudaMemcpy(h_u2, d_u1, bytes, cudaMemcpyDeviceToHost));
  checkCudaErrors(cudaMemcpy(h_du2, d_du1, sizeof(double) * NX * NY * NZ,
  ...
  checkCudaErrors(cudaFree(d_du2));
  free(h_u1);
  free(h_u2);
  free(h_du1);
  free(h_du2);
